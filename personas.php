...
<html lang="en">
  ...
  <body>
    ...
    <table class="table table-sm">
        ...
        <tbody>
            <?php
                $arrayFollowingRaw = $db->getFollowing($pdo, $_SESSION['id']);
                $arrayFollowing = [];
                foreach($arrayFollowingRaw as list($a)){
                    array_push($arrayFollowing, $a);
                }
                
                foreach($arrayFollowing as $idperson){
                    list($usr, $nam) = $db->getAccountData($pdo, $idperson);
                    if($db->isArtist($pdo, $idperson)){
                        $tipo = "Artista";
                    }
                    else{
                        $tipo = "Usuario";
                    ...
                    echo "<tr>
                    <td> <a href='viewprofile.php?ac=" . $idperson . "'>" . $usr . "</a></td>
                    <td>" . $nam . "</td>
                    <td>" . $tipo . "</td>
                    </tr>";
    ...
    <table class="table table-sm">
        ...
        <tbody>
            <?php
                $arrayAll = $db->getAllPeopleID($pdo, $_SESSION['id']);
                foreach($arrayAll as list($idperson)){
                    if(!in_array($idperson, $arrayFollowing)){
